...
function Crystal(
        ...
        while i <= length(lines)
            ...
            if line[1] == "_symmetry_space_group_name_H-M" ||
               line[1] == "_space_group_name_Hall"
                ...
                if space_group == "P1" || space_group == "P 1" || space_group == "-P1"
            ...
            if line[1] == "loop_"
                ...
                fractional =
                    fractional ||
                    haskey(name_to_column, "_atom_site_fract_x") &&
                ...
                cartesian =
                    cartesian ||
                    !fractional &&
                    haskey(name_to_column, "_atom_site_Cartn_x") &&
                ...
                if haskey(name_to_column, "_atom_site_Cartn_x") ||
                   haskey(name_to_column, "_atom_site_fract_x") # to have entered _atom_site loop
                ...
                elseif fractional && !atom_info
                    ...
                    while i <= length(lines) &&
                        length(split(lines[i])) == length(name_to_column)
                        line = split(lines[i])
                        ...
                        coords = [coords [
                            mod(
                                parse(
                                    Float64,
                                    split(line[name_to_column["_atom_site_fract_x"]], '(')[1]
                                ),
                                1.0
                            ),
                            mod(
                                parse(
                                    Float64,
                                    split(line[name_to_column["_atom_site_fract_y"]], '(')[1]
                                ),
                                1.0
                            ),
                            mod(
                                parse(
                                    Float64,
                                    split(line[name_to_column["_atom_site_fract_z"]], '(')[1]
                                ),
                                1.0
                            )
                        ]]
                        ...
                        if haskey(name_to_column, "_atom_site_charge")
                            push!(
                                charge_values,
                                parse(Float64, line[name_to_column["_atom_site_charge"]])
                            )
                        else
                        ...
                        if read_bonds_from_file
                            label_num_to_idx[line[name_to_column[species_column]]] =
                                length(species)
                        end
                        # iterate to next line in file
                        i += 1
                ...
                elseif cartesian && !atom_info
                    ...
                    while i <= length(lines) &&
                        ...
                        coords = [coords [
                            parse(
                                Float64,
                                split(line[name_to_column["_atom_site_Cartn_x"]], '(')[1]
                            ),
                            parse(
                                Float64,
                                split(line[name_to_column["_atom_site_Cartn_y"]], '(')[1]
                            ),
                            parse(
                                Float64,
                                split(line[name_to_column["_atom_site_Cartn_z"]], '(')[1]
                            )
                        ]]
                        # If charges present, import them
                        if haskey(name_to_column, "_atom_site_charge")
                ...
                elseif read_bonds_from_file &&
                    ...
                    while i <= length(lines) &&
                        ...
                        line = split(lines[i])
                        atom_one_idx =
                            label_num_to_idx[line[name_to_column["_geom_bond_atom_site_label_1"]]]
                        atom_two_idx =
                            label_num_to_idx[line[name_to_column["_geom_bond_atom_site_label_2"]]]
                        add_edge!(
                            bonds,
                            atom_one_idx,
                            atom_two_idx,
    ...
    if convert_to_p1 && !is_p1 && !read_bonds_from_file
        @info @sprintf(
            "Crystal %s has %s space group. I am converting it to P1 symmetry.
To prevent this, pass `convert_to_p1=false` to the `Crystal` constructor.\n",
    ...
    if check_neutrality
        if !neutral(crystal, net_charge_tol)
            error(
                @sprintf(
                    "Crystal %s is not charge neutral; net charge is %f e. Ignore
...
function wrap!(crystal::Crystal)
    ...
    return wrap!(crystal.charges.coords)
end
...
function xyz_filename(crystal::Crystal)
    return replace(replace(crystal.name, ".cif" => ""), ".cssr" => "") * ".xyz"
end

function write_xyz(
    crystal::Crystal,
    filename::AbstractString=xyz_filename(crystal);
    center_at_origin::Bool=false,
    kwargs...
)
    atoms = Atoms(crystal.atoms.species, Cart(crystal.atoms.coords, crystal.box)) # put in Cartesian
    if center_at_origin
        x_c = crystal.box.f_to_c * [0.5, 0.5, 0.5]
        atoms.coords.x .-= x_c
    end
    return write_xyz(atoms, filename; kwargs...)
end

function write_vtk(crystal::Crystal, filename::AbstractString; kwargs...)
    return write_vtk(crystal.box, filename; kwargs...)
end
...
function vtk_filename(crystal::Crystal)
    return replace(replace(crystal.name, ".cif" => ""), ".cssr" => "") * ".vtk"
end
...
function apply_symmetry_operations(crystal::Crystal)
    ...
    for sr in 1:nb_symmetry_ops
        ...
        for a in 1:(crystal.atoms.n)
            ...
            atoms.coords.xf[:, atom_id] .= [
                Base.invokelatest.(sym_rule[k], crystal.atoms.coords.xf[:, a]...) for
                k in 1:3
            ]
        end
        ...
        for c in 1:(crystal.charges.n)
            ...
            charges.coords.xf[:, charge_id] .= [
                Base.invokelatest.(sym_rule[k], crystal.charges.coords.xf[:, c]...) for
                k in 1:3
            ]
        end
    end
    ...
    open(filename, "w") do cif_file
        ...
        for i in 1:(crystal.atoms.n)
            ...
            @printf(
                cif_file,
                "%s    %s    ",
                string(crystal.atoms.species[i]) *
        ...
        if ne(crystal.bonds) > 0
            ...
            for edge in collect(edges(crystal.bonds))
                dxf =
                    crystal.atoms.coords.xf[:, edge.src] -
                    crystal.atoms.coords.xf[:, edge.dst]
                nearest_image!(dxf)
                @printf(
                    cif_file,
                    "%s    %s    %0.5f\n",
                    idx_to_label[edge.src],
                    idx_to_label[edge.dst],
                    norm(dxf)
                )
            end
        end
...
function write_cif(crystal::Crystal; kwargs...)
    return write_cif(crystal, String(split(crystal.name, ".cif")[1]); kwargs...)
end
...
function write_cssr(xtal::Crystal, filename::AbstractString; quiet::Bool=false)
    ...
    open(filename, "w") do f
        ...
        @printf(
            f,
            "%f %f %f SPGR = 1 P 1   OPT = 1\n",
            rad2deg(xtal.box.α),
            rad2deg(xtal.box.β),
            rad2deg(xtal.box.γ)
        )
        @printf(f, "%d 0\n0 xtal : xtal", xtal.atoms.n)
        for a in 1:(xtal.atoms.n)
            q = 0.0
            if xtal.charges.n != 0
                q = xtal.charges.q[a]
            end
            @printf(
                f,
                "\n%d %s %f %f %f 0 0  0  0  0  0  0  0  %f",
                a,
                xtal.atoms.species[a],
                xtal.atoms.coords.xf[:, a]...,
                q,
            )
        end
    end
...
function write_cssr(crystal::Crystal; kwargs...)
    return write_cssr(crystal, String(split(crystal.name, ".")[1]))
end
...
"""
function remove_duplicate_atoms_and_charges(
    crystal::Crystal,
    r_tol::Float64=0.1,
    ...
    charges =
        remove_duplicates(crystal.charges, crystal.box, true; r_tol=r_tol, q_tol=q_tol)
    return Crystal(
        crystal.name,
        crystal.box,
        atoms,
...
"""
function assign_charges(
    crystal::Crystal,
    species_to_charge::Dict{Symbol, Float64},
    net_charge_tol::Float64=1e-5
)
    if crystal.charges.n != 0
        @warn @sprintf(
            "Charges are already present in %s. Removing the current charges on the
crystal and adding new ones...\n",
    ...
    if !neutral(new_crystal, net_charge_tol)
        error(
            @sprintf(
                "Net charge of crystal %s = %f > net charge tolerance %f. If
 charge neutrality is not a problem, pass `net_charge_tol=Inf`\n",
                crystal.name,
                net_charge(new_crystal),
                net_charge_tol
            )
        )
    end
...
function Base.isapprox(c1::Crystal, c2::Crystal; atol::Real=0.0)
    ...
    sym2 = c2.symmetry
    symmetry_flag =
        (sym1.is_p1 == sym2.is_p1) &&
        (sym1.space_group == sym1.space_group) &&
        (sym1.operations == sym2.operations)
...
function Base.getindex(crystal::Crystal, ids::Union{Array{Int, 1}, UnitRange{Int}})
    ...
    for edge in edges(crystal.bonds)
        if edge.src in ids && edge.dst in ids
            ...
            set_props!(
                ...
                old_to_new[edge.src],
                old_to_new[edge.dst],
                props(crystal.bonds, edge.src, edge.dst)
            )
        end
    end
    if crystal.charges.n == 0
    ...
    elseif (crystal.charges.n == crystal.atoms.n) &&
        ...
        error(
            "for getindex(crystal), crystal must have 0 charges or an equal number of charges and atoms that share coordinates"
        )
    end
    return crystal
end
...
function Base.lastindex(crystal::Crystal)
    if (crystal.atoms.n == crystal.charges.n) || (crystal.charges.n == 0)
        ...
        error(
            "to index the crystal, it must have 0 charges or an equal number of charges and atoms"
        )
    end
end
...
function bounding_box(crystal::Crystal)
    return SVector{3}([SVector{3}(crystal.box.f_to_c[:, i]u"Å") for i in 1:3])
end

function boundary_conditions(::Crystal)
    return SVector{3, BoundaryCondition}([Periodic(), Periodic(), Periodic()])
end
