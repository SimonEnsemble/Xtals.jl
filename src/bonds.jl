...
function add_bonding_rules(bonding_rules::Array{BondingRule})
    br = rc[:bonding_rules]
    return rc[:bonding_rules] = vcat(bonding_rules, br)
end
...
"""
function loop_over_bonding_rules(
    bonding_rules::Vector{BondingRule},
    species_i::Symbol,
    species_j::Symbol,
    r::Float64
)
    ...
    for br in bonding_rules
        ...
        if br.species_i == :* && br.species_j == :*
            species_match = true
        elseif br.species_i == :* &&
               (species_i == br.species_j || species_j == br.species_j)
            species_match = true
        elseif br.species_j == :* &&
               (species_i == br.species_i || species_j == br.species_j)
            species_match = true
        elseif (species_i == br.species_i && species_j == br.species_j) ||
               (species_j == br.species_i && species_i == br.species_j)
            species_match = true
        end
        if species_match
...
function remove_bonds!(crystal::Crystal)
    while ne(crystal.bonds) > 0
        rem_edge!(
            crystal.bonds,
            collect(edges(crystal.bonds))[1].src,
            collect(edges(crystal.bonds))[1].dst
        )
    end
    return clear_vectors!(crystal)
end
...
function infer_bonds!(
    ...
    for i in 1:(crystal.atoms.n)
        ...
        for j in (i + 1):(crystal.atoms.n)
            bonding_flag, r, cross_boundary_flag = is_bonded(
                crystal,
                i,
                j,
                bonding_rules;
...
function write_bond_information(
    ...
    if ne(crystal.bonds) == 0
        @warn(
            "Crystal %s has no bonds present. To get bonding information for this
...
DATASET POLYDATA\nPOINTS %d double\n",
    ...
    if verbose
        @printf(
            "Saved bond information for crystal %s to %s.\n",
            crystal.name,
    ...
    for bond in collect(edges(xtal.bonds))
        if !(:distance ∈ keys(props(xtal.bonds, bond)))
            ...
            set_prop!(
                ...
                j,
                :distance,
                distance(xtal.atoms, xtal.box, i, j, true)
            )
        end
    end
end
...
function clear_vectors!(bonds::MetaGraph)
    ...
    return set_prop!(bonds, :has_vectors, false)
end
...
function calculate_bond_vectors!(xtal::Crystal)
    ...
    if has_prop(xtal.bonds, :has_vectors) && get_prop(xtal.bonds, :has_vectors)
        ...
        error(
            "Crystal $(xtal.name) already has vectors. Clear them with `clear_vectors!` first."
        )
        return
    end
    ...
    return set_prop!(xtal.bonds, :has_vectors, true)
end
