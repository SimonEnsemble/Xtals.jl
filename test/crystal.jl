...
@testset "Crystal Tests" begin
    ...
    write_cif(crystal, joinpath(get_global(:path_to_crystals), rewrite_filename))
    ...
    write_cif(crystal, joinpath(get_global(:path_to_crystals), rewrite_filename), fractional_coords=false) # cartesian
    crystal_reloaded = Crystal(rewrite_filename)
    strip_numbers_from_atom_labels!(crystal_reloaded) # TODO Arthur remove this necessity from write_cif
    @test isapprox(crystal, crystal_reloaded, atol=0.0001)
 #     rm(rewrite_filename) # clean up.
    rewrite_filename = "rewritten.cif"
    write_cif(crystal, joinpath(get_global(:path_to_crystals), rewrite_filename), number_atoms=false)
    crystal_reloaded = Crystal(rewrite_filename)
    @test isapprox(crystal, crystal_reloaded, atol=0.0001)
    ...
    write_cssr(xtal_cif)
    write_cssr(xtal_cif, joinpath(get_global(:path_to_crystals), da_xtalname * ".cssr"))
    ...
    write_cif(crystal, joinpath(get_global(:path_to_crystals), "high_precision_charges_test.cif")) # should write charges with more decimals
    ...
    write_cif(crystal, joinpath(get_global(:path_to_crystals), "high_precision_charges_test_not_neutral.cif"))
